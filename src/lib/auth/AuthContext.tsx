...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      const response = await apiPost<LoginResponse>('http://api.mor.org/api/v1/auth/login', { email, password });
  ...
  const register = async (email: string, name: string, password: string) => {
    try {
      ...
      const response = await apiPost<User>('http://api.mor.org/api/v1/auth/register', { 
        email, 
        name, 
        password, 
        is_active: true 
